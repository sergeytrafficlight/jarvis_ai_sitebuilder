...
# Скрипт для восстановления дампа AI настроек
...
echo "Восстановление дампа AI настроек..."
echo "База данных: $DB_NAME@$DB_HOST:$DB_PORT"
echo "Файл дампа: $DUMP_FILE"

# Запрашиваем подтверждение
read -p "Вы уверены, что хотите восстановить дамп? Это перезапишет существующие данные в таблицах core_aimodelssettings и core_systemprompts. (y/N): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ $? -eq 0 ]; then
    ...
    echo "Восстановлены таблицы: core_aimodelssettings, core_systemprompts"
else
    echo "Ошибка при восстановлении дампа"
    exit 1
fi